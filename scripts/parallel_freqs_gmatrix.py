...
output_filename = os.path.join(file_dir, "sfs_noconvert", base_filename + '_' + num_samples_str + '.freqs.tsv')
...
for sd_value, samples in samples_dict.items():
    freqs = np.sum(G[:,samples],1)/num_to_sample # get frquencies by summing over individuals
    #mut_afs,edg = np.histogram(freqs,bins=bin_edges)
    #print(mut_afs)
    #print(edg)
    #mut_afs = np.bincount(freqs,minlength=num_to_sample) # do a bin count on the freqs to get sfs
    #if len(mut_afs)<=num_to_sample:
    #    padded_afs = np.pad(mut_afs, (0, num_to_sample+1 - len(mut_afs)), 'constant')        
    #elif len(mut_afs)>num_to_sample+1:
    #    print(f"Strange happenings")
    #    print(mut_afs)
    #    padded_afs = [float('nan')] * (num_to_sample+1)
    #else:
    #    padded_afs = mut_afs
    #print(f"{len(padded_afs)} {sd_value}") 
    #print(len(mut_afs))
    freq_data[sd_value] = freqs

# Create DataFrame from the SFS data
#sfs_data['allele counts'] = bin_edges[1:len(bin_edges)]
#print(len(sfs_data['allele counts']))
freq_data["total"]=np.sum(G,1)/np.shape(G)[1]
freq_df = pd.DataFrame(freq_data)

# Save SFS DataFrame to file
print("Saving file to")
print(output_filename)
freq_df.to_csv(output_filename, sep='\t', index=False,na_rep= "NA")
