...
output_filename = os.path.join(file_dir, "sfs_log", base_filename + '_' + num_samples_str + '.sfs.tsv')
...
for sd_value, samples in samples_dict.items():
    ...
    print(f"St. dev for sk width {sd_value}: {np.std(locs[samples,0])}")
